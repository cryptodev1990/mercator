...
export class ModifyMode extends GeoJsonEditMode {
  getGuides(props: ModeProps<FeatureCollection>): GuideFeatureCollection {
    ...
    if (picks && picks.length && mapCoords) {
      ...
      const featureAsPick =
        !existingEditHandle && picks.find((pick) => !pick.isGuide);
      ...
      if (
        ...
        !(
          props.modeConfig?.lockRectangles &&
          featureAsPick.object.properties.shape === "Rectangle"
        ) &&
  ...
  getNearestPoint(
    ...
    return nearestPointOnLine(line, inPoint, viewport ? viewport : undefined);
  }
  ...
  _dragEditHandle(
    ...
    // @ts-ignore
    // const lengthOfGeom = editedFeature.geometry.coordinates[0].length;
    // TODO remove, the issue is not here
    // Selected feature is correct, length of new shape is already modified by time of arrival
    // toast.success(
    //   "geom length " +
    //     lengthOfGeom +
    //     " for features " +
    ...
    if (
      props.modeConfig?.lockRectangles &&
      editedFeature?.properties?.shape === "Rectangle"
    ) {
