// graft from https://raw.githubusercontent.com/uber/nebula.gl/master/modules/edit-modes/src/utils.ts
...
import {
  ...
  FeatureWithProps,
  AnyCoordinates,
} from "@nebula.gl/edit-modes/src/geojson-types";
...
export function nearestPointOnProjectedLine(
  ...
  return {
    ...
    geometry: {
      type: "Point",
      coordinates: wmViewport.unproject([x0, y0, z0]) as any,
    },
    properties: {
...
export function nearestPointOnLine(
  ...
  flattenEach(lines, (line: any) => {
    ...
    if (coords.length > 1 && pointCoords.length) {
      ...
      if (mercator) {
        lineCoordinates = coords.map((lineCoordinate: any) =>
      ...
      if (segmentIdx === undefined || to === undefined || from === undefined) {
        throw new Error("segmentIdx is undefined");
      }
      const dx =
    ...
    if (
      snapPoint === undefined ||
      snapPoint.x === undefined ||
      snapPoint.y === undefined ||
      snapPoint.from === undefined ||
      snapPoint.to === undefined
    ) {
      throw new Error("snapPoint is undefined");
    }
...
export function getPickedEditHandle(
  ...
  // 4ODO remove - handle type is "existing" if snapped rather than added
  // it is "intermediate" if added
  // issue is somewhere upstream
