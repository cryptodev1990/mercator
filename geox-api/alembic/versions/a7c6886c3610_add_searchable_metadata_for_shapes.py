...
def upgrade() -> None:
    op.add_column(
        ...
        sa.Column(
            "fts",
            postgresql.TSVECTOR(),
            sa.Computed("to_tsvector('english', properties - '__uuid')", persisted=True),  # type: ignore
            nullable=True,
        ),
    )
    op.create_index(
        "ix_properties_fts", "shapes", ["fts"], unique=False, postgresql_using="gin"
    )
    conn = op.get_bind()
...
def downgrade() -> None:
    op.drop_index("ix_properties_fts", table_name="shapes", postgresql_using="gin")
