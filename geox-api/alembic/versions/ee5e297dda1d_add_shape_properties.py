...
def upgrade() -> None:
    ...
    conn = op.get_bind()
    conn.execute("""
      UPDATE shapes SET geojson = (REGEXP_REPLACE(geojson::TEXT, '\\\\u0000', '\\\\u0002', 'g'))::JSONB;
      UPDATE shapes SET properties = (geojson->>0)::JSON->'properties' WHERE geojson->>0 IS NOT NULL;
      UPDATE shapes SET properties = geojson['properties'] WHERE geojson['properties'] IS NOT NULL;
    """)
