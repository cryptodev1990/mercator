...
@router.get("/osm/ways/")
async def get_roads_by_bounding_box(xmin: float, ymin: float, xmax: float, ymax: float, shape_to_snap: Feature):
    with OsmSessionLocal() as db_osm:
        db_osm.execute("""
        WITH all_ways AS (
        SELECT way
...
async def get_shapes_from_osm(latlngs: List[Tuple[float]], time_in_minutes: float, profile: str):
