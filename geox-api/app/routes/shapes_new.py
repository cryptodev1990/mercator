...
from app.dependencies_alt import UserConnection, get_user_connection
...
@router.post("/geofencer/new/shapes", response_model=GeoShape)
def create_shape(
...
@router.get("/geofencer/new/shapes", response_model=List[GeoShape])
def get_all_shapes(
...
@router.get("/geofencer/new/shapes/{uuid}", response_model=GeoShape)
def get_shape(
    uuid: UUID4,
    user_conn: UserConnection = Depends(get_user_connection),
) -> Optional[GeoShape]:
...
@router.put("/geofencer/new/shapes/{uuid}", response_model=GeoShape)
def update_shape(
...
@router.post("/geofencer/new/shapes/bulk", response_model=ShapeCountResponse)
...
@router.delete("/geofencer/new/shapes/bulk", response_model=ShapeCountResponse)
