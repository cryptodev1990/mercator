...
worker_id = ''.join([random.choice(ascii_letters + digits) for _ in range(settings.worker_id_length)])
...
def attach_stats_middleware(app: FastAPI):
    ...
    async def endpoint_stats(request: Request, call_next: Callable) -> Any:
        ...
        stats.increment(f"{endpoint}.response_status_code.{response.status_code // 100}XX")
        return response
