...
class SkipStripe(CLIApp):
    ...
    def generate_cli(
        ...
        parser.add_argument("--organization-id",
                            help="The organization to add to the whitelist")
        parser.add_argument(
            "--should-whitelist", help="Add to the whitelist (allowed values are true / false)",
        )
        return parser
    ...
    def run(self, args, loop: asyncio.BaseEventLoop) -> None:
        ...
        loop.run_until_complete(
            _whitelist_update(
                args.organization_id, args.should_whitelist
            )
        )
